...
// import { formatCurrency, formatCurrencyChange, formatPercentage, formatRelativeTime } from "@/lib/formatters";
...
export default function Dashboard() {
  ...
  // Temporarily use static challenge text
  const todayChallenge = "Invest ₹100 today to maintain your streak!";
  ...
  const handleRoundUpToggle = async () => {
    const newState = !roundUpEnabled;
    setRoundUpEnabled(newState);
    
    try {
      // In a real app, you would call the API to update user preferences
      // await apiRequest(API_ENDPOINTS.USERS(clerkId), {
      //   method: 'PATCH',
      //   body: JSON.stringify({ roundUpEnabled: newState })
      // });
      
      console.log('Round-up investment:', newState ? 'enabled' : 'disabled');
    } catch (error) {
      console.error('Error updating round-up preference:', error);
      setRoundUpEnabled(!newState); // Revert on error
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <RoundUpInvestment
          ...
          onTransactionProcessed={(transaction) => {
            console.log('Round-up transaction processed:', transaction);
            // You can add additional logic here like updating portfolio data
          }}
          className="mb-6"
        />
